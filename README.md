...
The browser UI is a SvelteKit app in `web/app`, using shadcn-svelte. Go serves that client for every page except the Pi transcript viewer, and the JSON API under `/api/v1`. Build the UI before `go run` if you want the real pages instead of the fallback shell:
