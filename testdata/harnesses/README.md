...
| --- | --- | --- |
| `pi-v3` | `pi-jsonl` | Pi session format 3, [`session-format.md`](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/docs/session-format.md) |
| `claude-code` | `claude-code-jsonl` | Claude Code JSONL transcript shape observed in current releases; public transcript access example in [`stop-hook.sh`](https://github.com/anthropics/claude-code/blob/main/plugins/ralph-wiggum/hooks/stop-hook.sh) |
...
| `amp-rich` | `amp-thread-export` | Invented content using numeric IDs, summary, tool-result, usage, origin, image, and environment shapes observed in exports on 2026-09-16; includes an unknown block to exercise fallback inspection |
| `amp-agents` | `amp-thread-export` | Invented viewer scenario covering a successful skill load, Task subagent answer, failed and successful shell calls, patch diff, spawned thread, and incoming thread reply; no internal subagent history is invented |
